export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!refreshToken) {
        console.error("[webhook] ERROR: refresh_token missing in supabase");
      ...
      if (tr.status !== 200) {
        console.error(
          "[webhook] strava token refresh failed:",
          JSON.stringify(trJson),
        );
      ...
      if (newRefreshToken && newRefreshToken !== refreshToken) {
        ...
        await fetch(`${SUPABASE_URL}/rest/v1/strava_token?user_id=eq.${userId}`, {
          method: "PATCH",
          headers: {
            apikey: SUPABASE_KEY,
            Authorization: `Bearer ${SUPABASE_KEY}`,
