export default async function handler(req, res) {
  ...
  const getDayName = dateStr => dayNames[new Date(dateStr + 'T12:00:00').getDay()];
  ...
  const actDetails = weekActs.map(a =>
    `• ${getDayName(a.date)} ${a.date} [${fmtType(a.type)}] ${a.name || ''}` +
  ...
  const racesInfo = upcoming.length === 0
    ...
    : upcoming.map(r => {
        const daysLeft = Math.round((new Date(r.date) - now) / 86400000);
        return `• ${r.date} ${r.name || ''}${r.goal ? ' · Ziel: ' + r.goal : ''} (in ${daysLeft} Tagen)`;
      }).join('\n');
