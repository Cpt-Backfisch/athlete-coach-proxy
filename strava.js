...
// Umgebungsvariablen setzen: STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET

export default async function handler(req, res) {
  // CORS — erlaubt Aufruf aus dem Browser (auch file://)
  ...
  if (action === 'exchange') {
    ...
    // Gibt zurück: access_token, refresh_token, expires_at, athlete
    return res.status(200).json(data);
  }

  // ── 2. Token erneuern (Refresh Token → neuer Access Token) ──
  if (action === 'refresh') {
  ...
  if (action === 'activities') {
    ...
    if (Date.now() / 1000 > expires_at - 60) {
      ...
      // Neuen Token zurückgeben damit die App ihn speichern kann
